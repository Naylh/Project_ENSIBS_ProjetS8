...
async def create_upload_file_obfuscation(input_file: UploadFile = File(...)):
    ...
    try :
        ...
        if input_file.content_type == "image/jpg" or input_file.content_type == "image/jpeg":
            convert_jpg_to_png(file_path, file_path[:-4] + ".png")
...
# Algorithme à changer suivant la convenance car ici les clés sont toujours les mêmes
...
def convert_jpg_to_png(jpg_path, png_path):
    """Convert a JPEG file to a PNG file.
    
    Args:
        jpg_path (str): path to the JPEG file
        output_path (str): path to the PNG file
    """
